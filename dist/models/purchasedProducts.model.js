"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const sequelize_1 = require("sequelize");
const connection_1 = __importDefault(require("../db/connection"));
const subscription_model_1 = __importDefault(require("./subscription.model"));
const PurchasedProduct = connection_1.default.define('purchased_products', {
    id: {
        type: sequelize_1.DataTypes.INTEGER,
        primaryKey: true,
        autoIncrement: true,
    },
    subscription_id: { type: sequelize_1.DataTypes.NUMBER },
    units: { type: sequelize_1.DataTypes.NUMBER },
    price_unit: { type: sequelize_1.DataTypes.DOUBLE },
    subtotal: { type: sequelize_1.DataTypes.DOUBLE },
    tax: { type: sequelize_1.DataTypes.DOUBLE },
    iva: { type: sequelize_1.DataTypes.DOUBLE },
    total: { type: sequelize_1.DataTypes.DOUBLE },
    product: { type: sequelize_1.DataTypes.STRING },
    is_deleted: { type: sequelize_1.DataTypes.BOOLEAN },
    is_active: { type: sequelize_1.DataTypes.BOOLEAN },
    is_integraded_subscription: { type: sequelize_1.DataTypes.BOOLEAN },
    created_at: { type: sequelize_1.DataTypes.DATE },
    updated_at: { type: sequelize_1.DataTypes.DATE },
});
PurchasedProduct.belongsTo(subscription_model_1.default, { foreignKey: 'subscription_id' });
subscription_model_1.default.hasMany(PurchasedProduct, { foreignKey: 'subscription_id' });
exports.default = PurchasedProduct;
//# sourceMappingURL=purchasedProducts.model.js.map