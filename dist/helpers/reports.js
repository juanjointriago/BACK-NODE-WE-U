 ...
 * @returns Una cuerda
 */
const createReportRequest = (requests, month, year, requestBuss) => {
    let total = 0;
    const wb = new xl.Workbook();
    const ws = wb.addWorksheet('Reporte');
    const ws2 = wb.addWorksheet('Empresas');
    const isMonst = (0, utils_1.getMonthName)(month);
    ...
    ws.cell(3, 2).string((0, moment_1.default)().format('DD/MM/YYYY HH:mm:ss')).style(styleDetail);
    ...
    ws2.cell(3, 2).string((0, moment_1.default)().format('DD/MM/YYYY HH:mm:ss')).style(styleDetail);
    const headingColumnNames = ['Nombre Empresa', 'Nombre Cliente', 'Conductor', 'Fecha Viaje', 'Tipo de Orden', 'Total'];
    //Write Column Title in Excel file
    let headingColumnIndex = 1;
    let rowAux2 = 5;
    headingColumnNames.forEach((heading) => ws2
            ...
            if (item.get().requests) {
                item.get().requests.length > 0 &&
                    item.get().requests.map((i, idx) => {
                        ...
                        ws2.cell(rowAux2, 5).string(`${(0, utils_1.getNameTyService)(i.idTypeService)}`);
